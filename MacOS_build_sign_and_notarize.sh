...
export PASS="$1"
export IDENTIFIER="ca.umontreal.musique.gris.audiodescriptors.pkg"

export PROJECT_PATH="`pwd`"
export PROJECT_FILE="$PROJECT_PATH/AudioDescriptors.jucer"
...
export ZIP_PATH="$TEMP_PATH/audiodescriptorsplugins.zip"
...
echo -e "$LINE\nVersion is $VERSION"
export PKG_PATH="$TEMP_PATH/AudioDescriptors_$VERSION.pkg"
...
#==============================================================================
function build() {
	echo -e "$LINE\nBuilding AudioDescriptors $VERSION\n$LINE"
	cd "$XCODE_PATH"
	chmod -R 755 .
...
function notarize_pkg()
{
	...
	xcrun notarytool submit --apple-id "$NOTARIZE_USER" \
							--password "$PASS" \
							--team-id $TEAM_ID \
							"AudioDescriptors_$VERSION.pkg" || exit 1
