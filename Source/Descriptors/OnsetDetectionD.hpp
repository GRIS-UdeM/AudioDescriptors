...
class OnsetDetectionD : public Descriptors {
	...
	void mOnsetDetectionProcess(juce::AudioBuffer<float>& descriptorBuffer, double sampleRate, int blockSize) {
        ...
        for (int i{}; i < allSamples.size() / numSamplesToProcess; ++i) {
            ...
            for (int k = 0; k < nOnsetFrames; k++) {
                ...
                onsetDectectionVals.push_back(mOnsetDetection->processFrame(window, mOnesetDetectionMetric, 1, 0 /*more than 0 gives assert*/, fluid::FluidDefaultAllocator()));
            }
        }
        ...
        if (allSamples.size() % numSamplesToProcess != 0) {
            ...
            for (int i = static_cast<int>(allSamples.size()) / numSamplesToProcess * numSamplesToProcess, j = 0; i < allSamples.size(); ++i, ++j) {
        ...
        if (mDescOnsetDetectionCurrent > 0) {
            if ((mOnsetDetectionDirection == Direction::up && mDescOnsetDetectionCurrent >= mDescOnsetDetectionTarget) ||
                (mOnsetDetectionDirection == Direction::down && mDescOnsetDetectionCurrent <= mDescOnsetDetectionTarget)) {
                mOnsetDetectionDirection = Direction::down;
                mDescOnsetDetectionTarget = 0.0;
                mDifferenceOnsetDetection = mDescOnsetDetectionTarget - mDescOnsetDetectionCurrent;
                mOnsetDetectionIncrement = (mDifferenceOnsetDetection / (mTimeToOnsetDetectionZero * (blockSize / sampleRate * 1000))) * 200;// 200 could be adjusted for smoothed value
            }
        }
        else {
