...
{
	if (mDescriptorToUse == DescriptorID::centroid || mDescriptorToUse == DescriptorID::spread || mDescriptorToUse == DescriptorID::noise) {
		if ((paramRangeCentroid != 0 && paramMaxFreqCentroid > paramMinFreqCentroid) ||
			(paramRangeSpread != 0 && paramFactorSpread > 0) ||
			(paramRangeNoise != 0 && paramFactorNoise > 0)) {
			return true;
		}
	}
	return false;
...
{
	if (mDescriptorToUse == DescriptorID::centroid && paramMinFreqCentroid < paramMaxFreqCentroid && paramRangeCentroid != 0) {
		return true;
	}
	return false;
}
